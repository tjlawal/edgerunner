...
if not "%shader_release%"=="1"							set shader_debug=1
...
if "%shader_debug%"=="1"		set shader_compile=%fxc_debug%		&& echo [shader compiled in debug mode]
if "%shader_release%"=="1" 	set shader_compile=%fxc_release%  && echo [shader compiled in release mode]
...
:: --- Build shaders
:: --- TODO: This current system compiles all shaders all the time.
:: --- it would be nice to not do the naive thing and be more smarter
:: --- about the shader compilation process.
 pushd run_tree
 	set SHADER_SRC_DIR=assets\shaders
 	if not exist data\shaders (mkdir data\shaders) 
 	set SHADER_OUT_DIR=data\shaders
 	for %%f in ("%SHADER_SRC_DIR%\*.hlsl") do (
  		set SHADER_FILE=%%~f
  		set SHADER_NAME=%%~nf
  		set SHADER_EXT=%%~xf

  		REM echo !SHADER_FILE!
  		REM echo !SHADER_NAME!
  		REM echo !SHADER_EXT!

  		set SHADER_PROFILE=
  		echo !SHADER_NAME! | findstr /I "_vs" >nul && set "SHADER_PROFILE=vs_5_0"
  		echo !SHADER_NAME! | findstr /I "_ps" >nul && set "SHADER_PROFILE=ps_5_0"
  		echo !SHADER_NAME! | findstr /I "_gs" >nul && set "SHADER_PROFILE=gs_5_0"

  		if not defined SHADER_PROFILE (
   			echo "[ERROR] Unknown shader type for !SHADER_NAME!. Add proper suffix (_vs, _ps, etc.)."
   			exit /b 1
  		)
  		REM echo !SHADER_PROFILE!
  		
  		%shader_compile% /T !SHADER_PROFILE! /E main %fxc_out% !SHADER_OUT_DIR!\!SHADER_NAME!.cso !SHADER_FILE! >nul || ( 
...
REM 	REM --- Delete previous build related files before compiling
	...
	if "%hello%"=="1"					set didbuild=1 && %compile% ..\src\hello.cc %compile_link% %link_resource% %out%edgerunner.exe || exit /b 1
	if "%triangle%"=="1"			set didbuild=1 && %compile% ..\src\triangle.cc %compile_link% %link_resource% %out%edgerunner.exe || exit /b 1
	if "%textured%"=="1"			set didbuild=1 && %compile% ..\src\textured.cc %compile_link% %link_resource% %out%edgerunner.exe || exit /b 1
